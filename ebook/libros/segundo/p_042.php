...
var arrverbs=[
	...
	["presenta","un","desierto","costero"],
	["ríos","de","corto","recorrido"],
	["permitiendo","la","agricultura","intensiva"],
	["1865","por","Paz","Soldán"],
	["accidentada","con","muchas","playas"],
	["esteros,","deltas","y","manglares"],
	["vegetación","de","las","lomas"],
	["de","clima","tropical","seco"],
	["cálidos","pero","muy","secos"]
]
arrrptas=[];
		...
		for(var k=0;k<targetX.length;k++)	{
			if (isWithin(targetX[k], x, x + sideLength) || isWithin(targetX[k] + targetLength, x, x + sideLength)) { // X
				if (isWithin(targetY[k], y, y + sideLength) || isWithin(targetY[k] + targetLength, y, y + sideLength)) { // Y
					document.getElementById("boom"+actuals++).play();
					actuals=(actuals>4 ? 0 : actuals);
					targetX[k] = -1;
					targetY[k] = -1;
...
function instructions() {
	...
	data+="<div style='float:left;width:40%;overflow:hidden;font:bold 28px verdana;text-align:center;padding:5px 0px;' id='' class='' ><img class='imagenrotar'  src='libros/"+top.document.getElementById("ruta").value+"/media/i_042.png'  style='width:100%;height;auto;'/></div>";
	data+="<div style='float:left;width:calc(60% - 20px);overflow:hidden;font:bold 28px verdana;text-align:center;padding:50px 10px;' id='' class='' >Dispara y derriba a cada bandido... pero debes hacerlo en orden, de manera que cada escuadron derribado forme una frase que se encuentra en el texto</div>";
...
function menu() {
  ...
  context.fillText('Derriba a los bandidos!', canvas.width / 2, canvas.height / 4);
  context.font = '24px Arial';
  context.fillText('Click para iniciar', canvas.width / 2, canvas.height / 2);
  context.font = '18px Arial'
  context.fillText('Use las flechas y espacio para apuntar y disparar', canvas.width / 2, (canvas.height / 4) * 3);
...
<body>
	...
	<div style="position:absolute;right:10px;bottom:10px;float:left;width:100%;overflow:hidden;text-align:right;" id="" class="" >
		<button id="btnsl" 	style="font:bold 38px verdana;border-radius:10px;background:#FF9900;text-shadow:-1px -1px 3px #fff;box-shadow:-1px -1px 1px #fff;display:none" onclick="pintarpta();parent.saveforlater('042',rpta)">Guardar para despues</button>
		<button id="btntd" 	style="font:bold 38px verdana;border-radius:10px;background:#FF9900;text-shadow:-1px -1px 3px #fff;box-shadow:-1px -1px 1px #fff;display:none" onclick="pintarpta();parent.taskdone('042',rpta)">Tarea terminada!</button>
	</div>
</body>
