...
window.onload=function() {
	init();
	instructions();
  words = ['corteza','montañas','mesetas','llanuras','depresiones','relieves','elevaciones','altiplanicies','altitudes','extensiones','aluviales','sedimentacion','penillanuras','absolutas','relativas'];
...
function instructions() {
	...
	data+="<div style='float:left;width:calc(60% - 20px);overflow:hidden;font:bold 28px verdana;text-align:center;padding:50px 10px;' id='' class='' >Resuelve el siguiente pupiletras.</div>";
...
function restore(cualrpta)	{
	...
	arrrpta=rpta.split("|");
// console.clear();
// console.info("%c["+new Date().toLocaleString()+"."+new Date().getUTCMilliseconds()+"]"+arguments.callee.name,"color:#008;font:bold 9px verdana;",gamePuzzle, arrrpta,rpta);
...
function respuesta(valor)	{
	...
	return(String(100*total/15).formatMoney(2)+"%");
}
...
<div id='words'  style="float:left;width:30%;font-size:22px;overflow:auto;"></div>
	...
	<div style="position:absolute;right:10px;bottom:10px;float:left;width:100%;overflow:hidden;text-align:right;" id="" class="" >
		<button id="btnsl" 	style="font:bold 38px verdana;border-radius:10px;background:#FF9900;text-shadow:-1px -1px 3px #fff;box-shadow:-1px -1px 1px #fff;display:none" onclick="parent.saveforlater('010',rpta)">Guardar para despues</button>
		<button id="btntd" 	style="font:bold 38px verdana;border-radius:10px;background:#FF9900;text-shadow:-1px -1px 3px #fff;box-shadow:-1px -1px 1px #fff;display:none" onclick="parent.taskdone('010',rpta)">Tarea terminada!</button>
	</div>
	<audio id="n0"><source src="media/n_100.wav"></audio>
	<audio id="n1"><source src="media/n_101.wav"></audio>
	<audio id="n2"><source src="media/n_102.wav"></audio>
	<audio id="n3"><source src="media/n_103.wav"></audio>
	<audio id="n4"><source src="media/n_104.wav"></audio>
	<audio id="n5"><source src="media/n_105.wav"></audio>
<script>
</script>
</body>
</html>