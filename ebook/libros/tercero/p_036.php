...
function instructions() {
	...
	data+="<div style='float:left;width:calc(60% - 20px);overflow:hidden;font:bold 28px verdana;text-align:center;padding:50px 10px;' id='' class='' >Drücken Sie die Taste und singen Sie das Lied. Du wirst deine erste Platte haben!</div>";
...
function respuesta(valor)	{
	...
	return(String(100*cantidad/arr.length).formatMoney(2)+"%");
}
...
function reset() {
	recognizing = false;
	document.getElementById("start").innerHTML = "ANFANG Aufzeichnung";
}
function toggleStartStop() {
	if (recognizing) {
		...
		recognition.start();
		recognizing = true;
		document.getElementById("start").innerHTML = "STOP Aufzeichnung";
...
<body>
	<div style="float:left;width:100%;overflow:hidden;padding:20px 0px;font-size:120px;text-align:center;color:#ccf;text-shadow:1px 1px 1px #009,1px 2px 2px #00f,2px 2px 10px #008;" id="" class="fontsports" >Sing a Song</div>
	<div style="float:left;width:calc(100% - 100px);height:400px;padding:50px;" id="" class="" >
		<div style="float:left;width:100%;overflow:hidden;" id="" class="" ><textarea style="width:100%;height:250px;background:rgba(255,255,255,.5);color:#f00;font:32px verdana;" id="txt" readonly></textarea>	</div>
		<div style="float:left;width:100%;overflow:hidden;padding:10px 0px;" id="" class="" >
			<button id="start" 		onclick="toggleStartStop()" 	style="padding:5px 0px;color:#000;background:#fa0;width:calc(50% - 6px);" class="">ANFANG Aufzeichnung</button>
			<button id="play" 		onclick="playtune()" 					style="padding:5px 0px;color:#000;background:#fa0;width:calc(25% - 6px);" class="">Abspielen</button>
			<button id="restart" 	onclick="limpiar()" 					style="padding:5px 0px;color:#000;background:#fa0;width:calc(25% - 6px);" class="">Klar</button>
		</div>
	...
	<div style="position:absolute;right:10px;bottom:10px;float:left;width:100%;overflow:hidden;text-align:right;" id="" class="" >
		<button id="btnsl" 	style="font:bold 42px verdana;border-radius:10px;background:#FF9900;text-shadow:-1px -1px 3px #fff;box-shadow:-1px -1px 1px #fff;display:none" onclick="parent.saveforlater('036',rpta)">Für später speichern</button>
		<button id="btntd" 	style="font:bold 42px verdana;border-radius:10px;background:#FF9900;text-shadow:-1px -1px 3px #fff;box-shadow:-1px -1px 1px #fff;display:none" onclick="parent.taskdone('036',rpta)">Aufgabe erledigt!</button>
	</div>
